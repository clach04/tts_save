## from googlw cache http://72.14.253.104/search?q=cache:oBgEMc62A7MJ:www.demonseed.net/~jp/code/magic.py+magic.py&hl=en&ct=clnk&cd=1&gl=us
## of http://www.demonseed.net/~jp/code/magic.py
#!/usr/bin/env python 
'''
magic.py
...
magic += [[0, 'string', '=', b'ID3', 'audio/mpeg']]
...
#############
"""
for x, row in enumerate(magic):
  if isinstance(row[3], str):
    magic[x][3] = bytes(row[3])
...
for x, row in enumerate(magic.magic):
  if isinstance(row[3], str):
    magic.magic[x][3] = bytes(row[3])

for row in magic.magic:
  if isinstance(row[3], str):
    row[3] = bytes(row[3])
...
f = open('/tmp/magic.pickle', 'wb')
pickle.dump(magic.magic, f)
f.close()


f = open('/tmp/magic.pickle', 'rb')
x = pickle.load(f)
f.close()

f = open('/tmp/magic.pickle', 'rb')
x = pickle.load(f, fix_imports=False)
f.close()


# only thing that works - but do not get bytes object
f = open('/tmp/magic.pickle', 'rb')
magic = pickle.load(f, encoding='latin1')
f.close()

for x, row in enumerate(magic):
  if isinstance(row[3], str):
    magic[x][3] = bytes(row[3], 'latin1')
